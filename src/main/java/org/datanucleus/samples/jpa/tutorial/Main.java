...
public class Main
{
        ...
        java.io.IOException, Exception
    {
        ...
        EntityManagerFactory emf =
            Persistence.createEntityManagerFactory("Tutorial", props);
    ...
    public static String findSSLRootCertFile(Class<?> mainClass) throws Exception {
        InputStream stream = mainClass.getResourceAsStream("/META-INF/persistence.xml");
        if (stream == null) {
            throw new IllegalStateException("Cannot find /META-INF/persistence.xml in classpath");
        }
        ...
        XPathExpression expr = xpath.compile(
            "/persistence/persistence-unit/properties/property[@name='javax.persistence.jdbc.sslrootcert']"
        );

        NodeList nodes = (NodeList) expr.evaluate(doc, XPathConstants.NODESET);
        if (nodes.getLength() == 0) {
            throw new IllegalStateException(
                "Property javax.persistence.jdbc.sslrootcert not found in persistence.xml"
            );
        }
